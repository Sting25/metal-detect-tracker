...
(function () {
    ...
    function renderMySuggestions(suggestions) {
        ...
        suggestions.forEach(function (s) {
            ...
            html += '<span style="font-weight:600;font-size:0.9rem;">' + escapeHtml(typeLabel) + '</span>';
            html += '<span style="font-size:0.8rem;font-weight:600;padding:2px 8px;border-radius:10px;background:' + statusColor + ';color:#fff;text-transform:uppercase;">' + escapeHtml(s.status) + '</span>';
            html += '</div>';
            html += '<p style="margin:0 0 0.25rem;font-size:0.85rem;color:var(--color-text, #333);">' + escapeHtml(s.suggested_text.length > 100 ? s.suggested_text.substring(0, 100) + '...' : s.suggested_text) + '</p>';
            if (s.admin_notes) {
                html += '<p style="margin:0 0 0.25rem;font-size:0.8rem;color:var(--color-text-muted, #888);font-style:italic;">Admin: ' + escapeHtml(s.admin_notes) + '</p>';
            }
            html += '<span style="font-size:0.75rem;color:var(--color-text-muted, #888);">' + escapeHtml(date) + '</span>';
            html += '</div>';
        });
        list.innerHTML = html;
    }
