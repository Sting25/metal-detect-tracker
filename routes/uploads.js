...
router.get('/:type/:filename', async function (req, res) {
    try {
        ...
        if (type === 'feedback') {
            ...
            var findPhotoRow = await db.queryOne(
            ...
            var find = findPhotoRow || await db.queryOne(
            ...
            if (req.user.role !== 'admin' && find.user_id !== req.user.id) {
                var findShare = await db.queryOne(
            ...
            if (req.user.role !== 'admin' && site.user_id !== req.user.id) {
                var siteShare = await db.queryOne(
                    'SELECT 1 FROM site_shares WHERE site_id = $1 AND shared_with_id = $2',
                    [site.id, req.user.id]
                );
                if (!siteShare) {
            ...
            var linkKey = type === 'signatures' ? 'signature_image_path' : 'signed_pdf_path';
            var linkRow = await db.queryOne(
            ...
            if (req.user.role !== 'admin' && perm.user_id !== req.user.id) {
                if (perm.site_id) {
                    var permShare = await db.queryOne(
                        'SELECT 1 FROM site_shares WHERE site_id = $1 AND shared_with_id = $2',
        ...
        var presignedUrl = await s3.getPresignedUrl(s3Key, 900); // 15 minutes
